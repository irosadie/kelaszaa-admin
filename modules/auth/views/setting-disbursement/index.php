...
$this->title = 'Master Pengaturan Pencairan Dana';
...
<div class="index">
    <div class="row">
        <div class="col-12">
            <p>
                <?= Html::a('<i class="fa fa-plus"></i> Master Pengaturan', ['create'], ['class' => 'btn btn-info m-1']) ?>
            </p>
            <div class="card">
                            ...
                            'afterRow' => function ($model, $key, $index, $grid) use ($month) {
                                ...
                                foreach ($model->disbursementPlans as $iskey => $isvalue) :
                                    ...
                                    $content_tabs[] = '<div class="tab-pane fade show ' . ($iskey == 0 ? "active" : "") . '" id="disbursement-' . ($model->id) . "-" . ($iskey + 1) . '" role="tabpanel" aria-labelledby="disbursement">
                                                            ' . isInput($key, 'Nama Pencairan', 'text', $isvalue->name, 'name[' . $isvalue->id . ']', '8', true) . isInput($key, 'Persentasi Pencairan', 'number', $isvalue->percentage_estimate, 'percentage_estimate[' . $isvalue->id . ']', '3', true) . isCombobox($key, 'Estimasi Bulan', $isvalue->month, 'month[' . $isvalue->id . ']', '8', true, $month) . isCombobox($key, 'Status', $isvalue->status, 'status[' . $isvalue->id . ']', '4', true, [1 => 'Aktif', 0 => 'Tidak Aktif']) . '
                                                      </div>';
                                endforeach;
