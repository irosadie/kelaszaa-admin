...
use app\models\danabos\{
    ...
    generals\DisbursementMaster,
    searches\DisbursementMaster as DisbursementMasterSearch
};
...
class SettingDisbursementController extends Controller
{
    ...
    public function actionIndex(): string
    {
        if (Helper::in_arrays(["operator"], Yii::$app->session->get("user_grant")['levels'] ?? [])) :
            $searchModel    = new DisbursementMasterSearch();
            ...
            if ($model->load(Yii::$app->request->post())) :
                ...
                if ($model->save()) :
                    ...
                    for ($a = 0; $a < $model->disbursement_in_year; $a++) :
                        $disbursementPlan = new DisbursementPlan();
                        $disbursementPlan->disbursement_master_id = $model->id;
                        $disbursementPlan->name = 'Q' . ($a + 1);
                        $disbursementPlan->percentage_estimate = round(100 / $model->disbursement_in_year);
                        $disbursementPlan->amount_estimate = 0;
                        $disbursementPlan->save(false);
                    endfor;
    ...
    public function actionUpdateDisbursementPlan()
    {
        try {
            $data = Yii::$app->request->post();
            foreach ($data as $key => $value) :
                foreach ($value as $k => $v) :
                    $model = DisbursementPlan::findOne($k);
                    if ($key == "amount_estimate") :
                        $model->$key = str_replace(".", "", $v);
                    else :
                        $model->{$key} = $v;
                    endif;
                    $model->save(false);
    ...
    {
        $model = DisbursementMaster::find()->where(['id' => $id])
            ->andWhere(['is', 'deleted_at', new \yii\db\Expression('null')])
            ->one();
        if ($model !== null) :
            return $model;
    ...
    public function actionValidate($code = '')
    {
        ...
        if ($code) :
            $code = Yii::$app->encryptor->decodeUrl($code);
            $model = DisbursementMaster::findOne($code);
        endif;
        if ($model->load(Yii::$app->request->post())) :
            Yii::$app->response->format = Response::FORMAT_JSON;
    ...
    protected function generateCode($length)
    {
        ...
        $model = DisbursementMaster::findOne(['code' => $code]);
        if (!$model) :
            return $code;
        else :
